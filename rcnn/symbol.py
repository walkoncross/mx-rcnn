...
def get_faster_rcnn_test(num_classes=21, num_anchors=9):
    """
    Faster R-CNN with VGG 16 conv layers, for inference
    :param num_classes: used to determine output size
    :return: Symbol
    """
    data = mx.symbol.Variable(name="data")
    ...
    # R-CNN
    pool5 = mx.symbol.ROIPooling(
        name='roi_pool5', data=relu5_3, rois=rpn_roi, pooled_size=(7, 7), spatial_scale=0.0625)
    ...
    # reshape output
    cls_prob = mx.symbol.Reshape(data=cls_prob, shape=(config.TRAIN.IMS_PER_BATCH, -1, num_classes), name='cls_prob_reshape')
    bbox_pred = mx.symbol.Reshape(data=bbox_pred, shape=(config.TRAIN.IMS_PER_BATCH, -1, 4 * num_classes), name='bbox_pred_reshape')

    # group output
    group = mx.symbol.Group([rpn_roi, cls_prob, bbox_pred])
